...
Before retrieval is executed, the system applies a lightweight \textit{gatekeeper} model as an initial screening layer. In this project, the gatekeeper is a binary machine-learning classifier whose purpose is to decide whether an extracted payload candidate should be treated as \textit{normal} or \textit{suspicious} before the system spends resources on retrieval and LLM reasoning. This layer acts as a \textit{payload screening gate}, which is especially useful in web-security pipelines where most incoming requests are benign and only a smaller subset requires deeper semantic interpretation.

The implemented gatekeeper uses a Random Forest model configured with 220 estimators and a maximum tree depth of 28. Its input representation combines two feature families through a feature-union design. The first family is character-level TF-IDF using n-grams from 2 to 4 characters, which helps the model capture short suspicious lexical fragments commonly seen in payloads such as operators, delimiters, and encoded sequences. The second family is a set of handcrafted statistical features, including payload length, Shannon entropy, special-character ratio, non-alphanumeric ratio, digit ratio, whitespace ratio, slash ratio, and the count of percent-encoded fragments. This combination provides both local lexical sensitivity and higher-level structural signals.

Operationally, the gatekeeper sits between payload extraction and retrieval. Payloads that are predicted as benign do not enter the full retrieval-and-generation path, while suspicious payloads are forwarded to hybrid retrieval and LLM reasoning. This design is useful because the project does not require every request to be explained by the LLM; instead, the model is invoked selectively for inputs that are more likely to benefit from deeper semantic analysis. In effect, the gatekeeper reduces unnecessary retrieval calls, lowers downstream inference cost, and keeps the more expensive reasoning stage focused on inputs with higher security relevance.

The training dataset for this classifier was built from two sources. Malicious samples were collected from PayloadsAllTheThings and cover web-oriented attack families such as SQL Injection, Cross-Site Scripting, Path Traversal, and Command Injection. Benign samples were generated synthetically from realistic templates using Faker and additional traffic-like patterns such as ordinary headers, REST-style paths, UUIDs, hashes, and common structured identifiers. The dataset was balanced at a 1:1 class ratio, deduplicated, filtered to remove strings shorter than 2 characters or longer than 512 characters, and then split using an 80/20 stratified partition so that class proportions remain stable between training and evaluation.

The screening decision threshold is intentionally set to 0.35 rather than the default 0.50. This lower threshold biases the model toward higher recall so that suspicious payloads are less likely to be missed. The associated class weighting also penalizes false negatives more strongly by assigning greater cost to the suspicious class. This design choice matches the logic of the full pipeline: a small increase in false positives is acceptable because later retrieval and LLM reasoning can refine the judgment, whereas missed malicious payloads would never reach the deeper analysis stage.

From an ablation perspective, the gatekeeper forms the second layer in a staged analysis hierarchy. The first layer is a rule-only fast path based on static heuristics and known-safe patterns. The second layer is the current rule-plus-gatekeeper setting, where suspicious candidates are identified by the Random Forest classifier. The third layer adds retrieval so that suspicious inputs receive external context from the payload intelligence corpus. The final layer invokes the LLM to produce a structured verdict, explanation, and mitigation-oriented reasoning. This staged design makes the pipeline modular and allows performance, cost, and reasoning quality to be compared at multiple levels of sophistication.
